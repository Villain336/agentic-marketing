terraform {
  ...
  backend "s3" {
    bucket         = "supervisor-terraform-state"
    key            = "production/terraform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "supervisor-terraform-locks"
    encrypt        = true
  }
}
...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "CORS_ORIGINS", value = "https://${var.domain}" },
    ]
...
# CRITICAL-08 fix: Block all public access to ALB logs bucket
...
# CRITICAL-08 fix: Enable server-side encryption for ALB logs
