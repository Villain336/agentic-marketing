...
class TenantStore:
    ...
    def __init__(self):
        # Lock for all mutable state — protects against concurrent access
        # from async handlers running in the same event loop thread.
        # Uses threading.Lock since the store operations are fast (in-memory)
        # and don't yield to the event loop.
