...
_TRANSIENT_PATTERNS = ("timeout", "connection", "network", "503", "429", "502", "504")
...
async def _retry(fn, *args, max_attempts: int = 3, **kwargs):
...
async def save_campaign(campaign_id: str, user_id: str, memory: dict,
    ...
    try:
        ...
        await _retry(lambda: client.table("campaigns").upsert(data).execute())
        return True
    except Exception as e:
...
async def load_campaign(campaign_id: str) -> Optional[dict]:
    ...
    try:
        result = await _retry(
            lambda: client.table("campaigns").select("*").eq("id", campaign_id).single().execute()
        )
        return result.data
    except Exception as e:
...
async def load_user_campaigns(user_id: str, limit: int = DEFAULT_PAGE_SIZE,
                              offset: int = 0) -> list[dict]:
    """Load campaigns for a user with pagination."""
    client = _get_client()
    if not client:
        return []

    try:
        result = await _retry(lambda: (
...
async def load_events(campaign_id: str = "", event_type: str = "",
    ...
    try:
        def _query():
            q = client.table("events").select("*").order("created_at", desc=True).limit(min(limit, DEFAULT_PAGE_SIZE))
            if campaign_id:
