...
class DesignViewEngine:
    ...
    def export_html(self, canvas_id: str, responsive: bool = True) -> str:
        ...
        html_parts = [
            ...
            f"<title>{self._esc(canvas.name)}</title>",
            "<style>",
    ...
    def _esc(value: Any) -> str:
        """Escape a value for safe insertion into HTML attributes and content."""
        return html_mod.escape(str(value), quote=True)

    def _element_to_html(self, element: DesignElement) -> str:
        """Convert a design element to HTML with proper escaping."""
        esc = self._esc
        styles = "; ".join(f"{k.replace('_', '-')}: {esc(v)}" for k, v in element.styles.items())
        props = element.properties

        if element.type == "hero":
            return (
                f'<section class="section" style="{styles}">'
                f'<h1 style="font-size: {esc(props.get("font_size_headline", "48px"))}; margin-bottom: 16px;">'
                f'{esc(props.get("headline", ""))}</h1>'
                f'<p style="font-size: {esc(props.get("font_size_sub", "20px"))}; margin-bottom: 32px; opacity: 0.9;">'
                f'{esc(props.get("subheadline", ""))}</p>'
                f'<a href="{esc(props.get("cta_url", "#"))}" style="display: inline-block; padding: 16px 40px; '
                f'background: rgba(255,255,255,0.2); border-radius: 8px; color: white; text-decoration: none; '
                f'font-weight: 600;">{esc(props.get("cta_text", "Get Started"))}</a>'
                f'</section>'
            )
        elif element.type == "card":
            features_html = ""
            if "features" in props:
                features_html = "<ul>" + "".join(f"<li>{esc(f)}</li>" for f in props["features"]) + "</ul>"
            return (
                f'<div style="{styles}">'
                f'<h3>{esc(props.get("title", props.get("plan_name", "")))}</h3>'
                f'<p>{esc(props.get("description", ""))}</p>'
                f'{features_html}'
                f'</div>'
            )
        elif element.type == "button":
            return (
                f'<a href="{esc(props.get("url", "#"))}" style="{styles}; display: inline-block; '
                f'text-decoration: none; cursor: pointer;">{esc(props.get("text", "Click"))}</a>'
            )
        elif element.type == "text":
            return f'<p style="{styles}">{esc(props.get("content", ""))}</p>'
        elif element.type == "image":
            return f'<img src="{esc(props.get("src", ""))}" alt="{esc(props.get("alt", ""))}" style="{styles}; max-width: 100%;">'
        elif element.type == "nav":
            links = " ".join(f'<a href="#" style="margin: 0 16px; text-decoration: none; color: inherit;">{esc(l)}</a>'
                             for l in props.get("links", []))
            return (
                f'<nav style="{styles}; display: flex; align-items: center; justify-content: space-between;">'
                f'<strong>{esc(props.get("logo_text", ""))}</strong>'
                f'<div>{links}</div>'
                f'<a href="#" style="padding: 8px 20px; background: #667eea; color: white; border-radius: 6px; '
                f'text-decoration: none;">{esc(props.get("cta_text", "Sign Up"))}</a>'
                f'</nav>'
            )
        elif element.type == "footer":
            return (
                f'<footer style="{styles}">'
                f'<p>&copy; {esc(props.get("company_name", "Company"))} {datetime.utcnow().year}</p>'
                f'</footer>'
            )
        elif element.type == "form":
            fields = ""
            for field_def in props.get("fields", []):
                fields += (
                    f'<input type="{esc(field_def.get("type", "text"))}" '
                    f'placeholder="{esc(field_def.get("placeholder", ""))}" '
            ...
            return (
                ...
                f'{esc(props.get("submit_text", "Submit"))}</button>'
                f'</form>'
            )

        return f'<div style="{styles}"><!-- {esc(element.type)} --></div>'
    ...
    def _element_to_jsx(self, element: DesignElement) -> str:
        """Convert a design element to JSX with proper escaping."""
        esc = self._esc
        styles = ", ".join(
            f'"{k}": "{esc(v)}"' for k, v in element.styles.items()
        )
        props = element.properties
        ...
        if element.type == "hero":
            return (
                ...
                f'<h1>{esc(props.get("headline", ""))}</h1>'
                f'<p>{esc(props.get("subheadline", ""))}</p>'
                f'<a href="{esc(props.get("cta_url", "#"))}">{esc(props.get("cta_text", ""))}</a>'
                f'</section>'
            )
        elif element.type == "button":
            return f'<a href="{esc(props.get("url", "#"))}" style={{{{{styles}}}}}>{esc(props.get("text", ""))}</a>'
        else:
            return f'<div style={{{{{styles}}}}}>/* {esc(element.type)} */</div>'
